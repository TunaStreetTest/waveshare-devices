 ...
 */
/*
 * Overlay of esp-brookesia's LVGL event binding for the Waveshare
 * ESP32-S3-Touch-AMOLED-1.8 V2.
 *
 * SINGLE DELTA FROM UPSTREAM: a node that declares a `gesture` event now has
 * LV_OBJ_FLAG_GESTURE_BUBBLE cleared, and gets it back when that binding goes
 * away. Everything else in this file is upstream verbatim.
 *
 * Why (#220 -- "swipes are still bad ... tap and swipe collision"):
 *
 * LVGL delivers a swipe to ONE object, chosen by climbing from the object
 * under the finger while the flag is set:
 *
 *     while(gesture_obj && lv_obj_has_flag(gesture_obj, LV_OBJ_FLAG_GESTURE_BUBBLE))
 *         gesture_obj = lv_obj_get_parent(gesture_obj);      // lv_indev.c
 *
 * and every object created with a parent gets the flag by default
 * (lv_obj.c:593 `if(parent) obj->flags |= LV_OBJ_FLAG_GESTURE_BUBBLE`). A
 * mounted JSON-UI document is a child of the active screen
 * (backend.cpp, lv_obj_set_parent), so the climb ran straight past every app
 * node to the parent-less LVGL screen -- and a `gesture` event declared in a
 * screen.json NEVER fired. tunastreet.xviewer's README predicted exactly this
 * in #183 and both apps kept half-panel tap zones as the "guaranteed path"
 * because of it; those zones sit where the finger drags, take the press, and
 * fire on pressed AND released, which is the collision Steven reported.
 *
 * Clearing the flag on the declaring node stops the climb there, which is the
 * behaviour the JSON-UI docs already describe (`events.rst`). It does not
 * touch the system's own gestures: the swipe-up-to-home / status-bar peek run
 * on raw touch through DisplayService::set_touch_gesture_config
 * (shell_display.cpp), not on LV_EVENT_GESTURE.
 */
...
static void unbind_events(Record &record)
{
    ...
    if (record.object != nullptr) {
        for (const auto &context : *record.event_contexts) {
            ...
            if (context->type == EventType::Gesture && lv_obj_get_parent(record.object) != nullptr) {
                // Restore the default the node was created with. Guarded on
                // having a parent so a screen/layer object is never handed a
                // flag LVGL deliberately withholds from it.
                lv_obj_add_flag(record.object, LV_OBJ_FLAG_GESTURE_BUBBLE);
            }
...
void bind_events(BackendImpl &impl, Record &record, const std::vector<EventBinding> &events)
{
    ...
    for (const auto &event : events) {
        ...
        if (event.type == EventType::Gesture) {
            // Stop the gesture climb here -- see the delta note at the top.
            lv_obj_remove_flag(record.object, LV_OBJ_FLAG_GESTURE_BUBBLE);
        }
        if (record.event_contexts == nullptr) {
