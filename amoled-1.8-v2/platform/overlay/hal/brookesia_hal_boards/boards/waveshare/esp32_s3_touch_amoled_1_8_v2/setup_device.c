...
#define LCD_GAP_X (16)  // CO5300 on the 1.8 V2 panel: 368 px window starts at column 16

/* Init table proven on this exact panel by the Phase 1 colorbar bring-up
 * (copied from xiaozhi-esp32 boards/waveshare/esp32-s3-touch-amoled-1.8-v2) */
...
esp_err_t lcd_panel_factory_entry_t(esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config, esp_lcd_panel_handle_t *ret_panel)
{
    ...
    // Panel power/reset pulse through the expander BEFORE panel init (proven order)
    ...
    ret = esp_lcd_new_panel_co5300(io, &panel_dev_cfg, ret_panel);
    if (ret != ESP_OK) {
    ...
    ret = esp_lcd_panel_set_gap(*ret_panel, LCD_GAP_X, 0);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Set co5300 panel gap failed");
        return ret;
    }
...
esp_err_t lcd_touch_factory_entry_t(esp_lcd_panel_io_handle_t io, const esp_lcd_touch_config_t *touch_dev_config, esp_lcd_touch_handle_t *ret_touch)
{
    ...
    ret = esp_lcd_touch_new_i2c_cst816s(io, touch_dev_config, ret_touch);
    if (ret != ESP_OK) {
